...
export const onRequestPost = async (context: { env: Env; params: { id: string }; request: Request }) => {
  try {
    ...
    const existing = await env.DB.prepare(
      `SELECT id FROM reactions WHERE target_type = 'reply' AND target_id = ? AND user_id = ? AND emoji = ?`
    ).bind(replyId, userId, emoji).first();
    ...
    if (existing) {
      ...
      await env.DB.prepare(
        `INSERT INTO reactions (id, target_type, target_id, user_id, emoji, created_at) VALUES (?, 'reply', ?, ?, ?, ?)`
      ).bind(rxId, replyId, userId, emoji, new Date().toISOString()).run();
    }

    // Return updated reactions for this reply
    const allReactions = await env.DB.prepare(
      `SELECT emoji, user_id FROM reactions WHERE target_type = 'reply' AND target_id = ?`
    ).bind(replyId).all();
